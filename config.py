...
# 분봉 해상도 (1분봉 통합 재설계)
# ──────────────────────────────────────────────────────────
# 박스 σ와 거래량 진동을 서로 다른 해상도로 산출하는 하이브리드 구성.
#   · σ(박스): 5분봉 유지 — 1분봉은 호가 bid-ask bounce 등 마이크로구조 노이즈로
#     실현변동성(RV)이 과대평가될 위험이 있어 5m가 안정적.
#   · 거래량(진동): 1분봉 — 5분 캔들이 흡수하던 캔들 내부 왕복을 포착.
#     해상도 스케일링 실측: osc ∝ Δt^-0.60 → 1m은 5m 대비 약 2.7배 진동 포착.
# 요청 해상도의 분봉이 DB에 없으면 자동으로 5m → 일봉 순서로 폴백한다.
INTRADAY_SIGMA_UNIT = 5         # σ 산출용 분봉 단위 (1,3,5,…). 권장 5
INTRADAY_VOLUME_UNIT = 1        # 거래량 진동 산출용 분봉 단위. 권장 1
