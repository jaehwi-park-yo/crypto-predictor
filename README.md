...
3. **원화 보유 비중** — KRW 현금을 헷지/물타기 예비로 보유 (`KRW_HOLD_RATIO`, 기본 30%). USDT-KRW는 트리플 오버레이(1σ 1/1·1.5σ 1/2·2σ 1/3원) 고정 전략으로 운영.
...
**자동 업데이트**: 서버가 GitHub 저장소의 최신 버전을 주기적으로 확인해,
새 버전이 있으면 대시보드 좌하단에 업데이트 배지가 표시됩니다. 클릭 한 번으로
다운로드·적용·서버 재시작까지 자동 수행됩니다 (data/ 폴더는 보존).

> ⚠️ 본 앱의 모든 수치는 통계적 **추정치**이며 투자 권유가 아닙니다. 투자 손실 책임은 본인에게 있습니다.

## 분봉 데이터셋 (F1 개선용)

**데이터셋 시드 내장**: 저장소에 `data/dataset_seed.zip`(약 20MB — BTC 5분봉 91만 행 + USDT 21만 행 + 일봉 + 환율 + 달러BTC)이 포함되어 있어, 최초 기동 시 별도 조치 없이 약 10초 만에 자동 복원됩니다. 이후에는 마지막 시점 이후 누락분만 증분 수집합니다 (수 시간 걸리던 분봉 부트스트랩 생략).

서버(`api_server.py`) 최초 부팅 시 업비트 공개 API에서 BTC/KRW·USDT/KRW **5분봉**을 시드 마지막 시점 이후부터 증분 수집합니다 (시드가 없으면 최대 약 2년치 부트스트랩 — 백그라운드 5~10분).
