...
> 또한 최초 부팅 시 분봉 데이터 수집이 백그라운드에서 5~10분 진행됩니다 — 대시보드는 그동안에도 즉시 사용 가능합니다.
...
API 서버(`start.bat`)가 실행 중이면 대시보드가 **60초 주기로 자동 갱신**됩니다.
...
→ `start.bat`로 API 서버를 먼저 실행하세요. 또는 파일을 직접 열어도 차트는 표시되며, 실시간 가격만 폴백 데이터로 표시됩니다.

**Q. 포트 8000이 이미 사용 중이라고 나와요**
→ 이미 API 서버가 실행 중입니다. 기존 "API Server" 창을 닫고 다시 실행하세요.
...
→ 가상환경이 활성화된 상태에서 `pip install pandas`를 실행하거나, `start.bat`를 통해 환경을 재설치하세요.
